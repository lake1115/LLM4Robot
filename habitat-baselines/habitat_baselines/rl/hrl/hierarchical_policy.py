...
class HierarchicalPolicy(nn.Module, Policy):
    ...
    def act(
        ...
        for i in range(self._num_envs):
            if actions[i, self._stop_action_idx] > 0.0: ## means nav success
                actions[i, self._stop_action_idx] = 0.0
        #actions[:, self._stop_action_idx] = 0.0 ## the stop actions dim is important in nav now
