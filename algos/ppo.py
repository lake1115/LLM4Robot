...
class PPO(Base):
    ...
    def save(self, file_name: str = "acmodel"):
        try:
            os.makedirs(self.save_path)
        except OSError:
            pass
        filetype = ".pth" # pytorch model
        torch.save(self.model, os.path.join(self.save_path, file_name + filetype))
        torch.save(self.model, os.path.join(self.save_path, "latest" + filetype))

    def update_policy(self, buffer):
        losses = []
        for _ in range(self.epochs):
            for batch in buffer.sample(self.minibatch_size, self.recurrent):
                obs_batch, action_batch, return_batch, advantage_batch, values_batch,  mask, log_prob_batch, skill_batch = batch
                pdf, value = self.model(obs_batch,skill_batch)
