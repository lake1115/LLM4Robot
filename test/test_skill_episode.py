...
def insert_render_options(config):
    ...
    with habitat.config.read_write(config):
        ...
        agent_config.sim_sensors.update(
            {"third_rgb_sensor": ThirdRGBSensorConfig(height=128, width=128)}
        )
    return config
...
def main(args):
    ...
    hidden_state_shape = (agent.actor_critic.num_recurrent_layers,agent._ppo_cfg.hidden_size)
    ...
    if args.show:
        ...
        try:
            ...
            pass
        video_writer = vut.get_fast_video_writer(video_file_path + "/episode-" + test_episode +".mp4", fps=60)
    env = trainer.envs
    with torch.no_grad():
        done = False
        ...
        while not done:
           ...
           # obs, reward, done, info = env.step(step_data)  # noqa: F841
            ...
            if args.show:
                ...
                # frame = frame[:,:,::-1]
                # cv.imwrite("data/video_dir/"+args.task+f"/{count_steps}.png",frame)
            done = dones[0]
            count_steps += 1
